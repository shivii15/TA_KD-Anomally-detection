...
def main():
    ...
    parser.add_argument('--save_name', type=str, default="Student_TGKD_SOTA")
    ...
    if args.teacher_type == 'resnet':
        teacher = TeacherResNet(X.shape[1], len(le.classes_))
    else:
        teacher = TeacherDNN(X.shape[1], len(le.classes_))
    ...
    # Initialize Anomaly Detector (Handles dictionary or raw model)
    ...
    for epoch in range(1, args.epochs + 1):
        student.train()
        total_loss = 0
        train_pbar = tqdm(train_loader, desc=f"🚀 Distill Epoch {epoch}", leave=False)
        ...
        for batch_x, batch_y in train_pbar:
            ...
            with torch.no_grad():
                ...
                # CPU-bound Isolation Forest call
            ...
            loss_feat = args.feat_weight * F.mse_loss(s_feat, t_feat)
        ...
        if val_acc > best_acc:
            ...
            print(f"⭐ New Best Student Saved: {val_acc:.2f}%")
