...
# ---------------------------
# ✅ SAFE GLOBALS FIX (PyTorch 2.6 + NumPy)
# ---------------------------
import sklearn
...
def load_teacher_model(args, input_dim, num_classes, device):
    ...
    # ⚠️ Use weights_only=False because checkpoint contains numpy/sklearn objects
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_x, batch_y in train_loader:
            ...
            with torch.no_grad():
                ...
                raw_a = torch.tensor(
                    iso_model.decision_function(batch_x.cpu().numpy()),
                    device=device
                )
            ...
            # 2. KD Loss (adaptive temperature)
            ...
            # Total Loss
            total_loss = loss_ce + loss_kd + (args.feat_weight * loss_feat)
    ...
    # ---------------------------
    # 5. SAVE MODEL (CLEAN FORMAT)
    # ---------------------------
    torch.save({
        'model_state_dict': student.state_dict(),
        'classes': le.classes_,   # ✅ instead of full LabelEncoder
        'metadata': {
            'method': 'TGKD',
    ...
    print(f"\n✅ TGKD Student saved to {args.save_path}")
